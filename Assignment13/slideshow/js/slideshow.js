...
window.addEventListener('load', () => {
    ...
    // SET THE SPEED
    $('speedCtrl').onclick = () => {
        let userSpeed = parseInt(prompt(`The current speed is ${slideshow.getSpeed()} milliseconds. Please enter your desired speed in ms.`));
        if (userSpeed == false || isNaN(userSpeed) || userSpeed < 0) {
            alert('Please enter a numeral greater than 0.');
        } else {
            slideshow.setSpeed(userSpeed);
        }
    }
});